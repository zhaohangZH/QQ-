var renderZhao=(function () {
    var $Nav=$('.nav'),导航栏盒子
        ...
        $Frame=$Release.find('.frame'),
        $Review=$Release.find('.review');

    //导航栏li下拉列表
    function navList(){
        $Navlist.each(function () {
            $(this).mouseenter(function () {
                $(this).children('.waibu').css('display')==='none'?$(this).children('.waibu').slideDown(150)&&$(this).css('background','#fff')&&$(this).children('a').css('color','#000'):null;
            });
            $(this).mouseleave(function () {
                $(this).children('.waibu').css('display')==='block'?$(this).children('.waibu').slideUp(150)&&$(this).css('background','')&&$(this).children('a').css('color',''):null;
    ...
    function faceFlip() {
        $flipa.each(function () {
            $(this).click(function () {
                ...
                $flipp.html()==1?$flipa.eq(0).addClass('no'):$flipa.eq(0).removeClass('no');
                $flipp.html()==5?$flipa.eq(1).addClass('no'):$flipa.eq(1).removeClass('no');
                $Bq.eq($flipp.html()-1).siblings().removeClass('bqk');
                $Bq.eq($flipp.html()-1).addClass('bqk');
            });
        })
    }
    //发表框
